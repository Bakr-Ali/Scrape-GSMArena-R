...
=======
  switch_vpn()
  src <- read_html(oem_base_url); Sys.sleep(15)
>>>>>>> Stashed changes
...
oem_urls_count <- function(oem_base_url) {

  print(oem_base_url)
  items <- oem_urls_count_s(oem_base_url)
  
  if (length(items) != 0) {
    page_range <- 1:(items[length(items)] %>% as.numeric())
    # int [1:16]
    
    page_range[length(page_range)]
    print(page_range[length(page_range)])
  } else {
    print(1)
    1
  }
